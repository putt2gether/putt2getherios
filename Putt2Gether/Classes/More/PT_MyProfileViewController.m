//
//  PT_MyProfileViewController.m
//  Putt2Gether
//
//  Created by Sachin on 17/02/17.
...
@interface PT_MyProfileViewController ()<UITextFieldDelegate,
...
@implementation PT_MyProfileViewController
...
//    {
   ...
   // }
    ...
    if (screenHeight == 568) {
        ...
        self.heightDisplay.constant = 52;
        self.heightCcountry.constant = 52;
        self.heightEmail.constant = 52;
        self.heightProfileView.constant = 135;
        
        self.heightMobile.constant = 52;
        
        self.heightPass.constant = 52;
        self.heightHandicap.constant = 52;
        self.heightHomeCourseV.constant = 52;
...
-(void)textFieldDidChange:(UITextField *)txtFld
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //self.heightTableConstraint.constant = [_sortedArray count];
            ...
            if (!error)
            {
                ...
                if (responseData != nil)
                {
                    ...
                    if ([responseData isKindOfClass:[NSDictionary class]])
                    {
                        ...
                        modelPlayer.homeCourseName = dicInfo[@"golf_course_name"];
                        modelPlayer.homecourseID =[dicInfo[@"golf_course_id"] integerValue];
                        
                        //Update UI
                        
                        self.profilename.text = [modelPlayer.playerName uppercaseString];
                        self.homeCourseText.text = [modelPlayer.homeCourseName uppercaseString];
                        self.countryname.text = [modelPlayer.country uppercaseString];
                        self.countryCodeLabel.text = modelPlayer.countryCode;
                        self.mobilenumber.text = modelPlayer.mobile;
                        self.handicapField.text = [NSString stringWithFormat:@"%li",(long)modelPlayer.handicap];
...
//        }
     ...
     if ([self checkDisplayNameBlank]  && [self checkCountryBlank] && [self checkHandicapBlank] && [self checkMobileBlank]){
        if (_homeCourseText.text == nil) {
            
            _golfCourseID = 0;
        }
        
        MGMainDAO *mainDAO = [MGMainDAO new];
        NSDictionary *param = @{@"user_id":[NSString stringWithFormat:@"%li",modelPlayer.playerId],
                                @"full_name":modelPlayer.playerName,
                                @"email":modelPlayer.email,
                                @"contact_number":modelPlayer.mobile,
                                @"country_code":modelPlayer.countryCode,
                                @"country":modelPlayer.country,
                                @"password":self.passwordTF.text,
                               @"golf_course_id":[NSString stringWithFormat:@"%ld",(long)_golfCourseID],
                                @"handicap_value":[NSString stringWithFormat:@"%li",(long)modelPlayer.handicap],
                                @"profile_image":str,
                                @"version":@"2"};
        ...
        if (!error)
        {
            if (responseData != nil)
            {
                if ([responseData isKindOfClass:[NSDictionary class]])
                {
                    ...
                    if ([dicOutput[@"status"] isEqualToString:@"1"])
                    {
                        NSDictionary *dicFullName = dicOutput[@"Full Name"];
                        [[MGUserDefaults sharedDefault] setUserImage:dataToSaveLocal];
                        [self actionBack:nil];
                        ...
                        [[MGUserDefaults sharedDefault] setAccessToken:[dicFullName objectForKey:@"token"]];
