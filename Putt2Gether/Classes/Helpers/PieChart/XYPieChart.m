...
{
    if ([key isEqualToString:@" "] || [key isEqualToString:@"endAngle"]) {
        return YES;
    }
    else {
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if(self)
    {
        ...
        _labelColor = [UIColor whiteColor];
        _labelRadius = _pieRadius/2;
        _selectedSliceOffsetRadius = 3;//MAX(10, _pieRadius/10);
...
//        float labelAngle = interpolatedEndAngle - interpolatedStartAngle;
//        float labelX = _pieCenter.x + (_labelRadius) * cos(labelAngle);
//        float labelY = _pieCenter.y + (_labelRadius) * sin(labelAngle);
//        CGPoint point = CGPointMake(labelX, labelY);
//        NSValue *pointValue = [NSValue valueWithCGPoint:point];
//        [self.arrLocations addObject:pointValue];
...
- (SliceLayer *)createSliceLayer
{
    ...
    [pieLayer setStrokeColor:[UIColor whiteColor].CGColor];
    //pieLayer.borderColor = [UIColor whiteColor].CGColor;
    //pieLayer.borderWidth = 2.0;
    //pieLayer.masksToBounds = YES;
